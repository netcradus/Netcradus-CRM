...
const getLeadStats = async (req, res) => {
  ...
  try {
    const [
      ...
      Lead.countDocuments(),
      Lead.countDocuments({ status: "call_back" }),
      Lead.countDocuments({ status: "meeting_aligned" }),
      Lead.countDocuments({ convertedToDealId: { $ne: null } }),
...
const convertLeadToDeal = async (req, res) => {
  ...
  try {
    ...
    if (lead.convertedToDealId) {
      return res.status(400).json({ success: false, message: "Lead already converted to a deal" });
    }
  ...
  assignedTo: req.user?.name || "Unassigned",
