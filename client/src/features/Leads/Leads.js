...
function Leads() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      const res = await axios.get(apiUrl("/api/leads"), { ...getAuthConfig(), params: Object.fromEntries(searchParams.entries()) });
      setLeads(Array.isArray(res.data?.data) ? res.data.data : []);
      setPagination(res.data?.pagination || { totalLeads: 0, totalPages: 1, currentPage: 1, limit: 10 });
    } catch (e) { setError(e.response?.data?.message || "Failed to fetch leads."); }
  ...
  useEffect(() => { fetchLeads(); }, [searchParams]);
    ...
    <div className="dashboard-container" style={{ padding: "var(--space-6)", position: "relative" }}>
      ...
      <div className="nc-card">
        <table className="nc-table">
            ...
            <tr>
              {isSuperUser ? (
                ...
                <>
                  ...
                  <th>Created</th><th>Last Note</th><th>Last Call</th><th>Actions</th>
                </>
              )}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={isSuperUser ? 8 : 7} style={{ textAlign: "center", padding: "var(--space-8)" }}>Loading leads...</td></tr>
              ...
              <tr key={lead._id}>
                {isSuperUser ? (
                  ...
                  <>
                    <td>
                      <div style={{ fontWeight: "var(--font-semibold)" }}>{lead.name}</div>
                      <div style={{ fontSize: 10, color: "var(--color-text-muted)" }}>{lead.company || "--"}</div>
                    </td>
                    <td>{lead.phone || "--"}</td>
                    <td><span className={`badge badge-${statusBadgeVariant(lead.status)}`}>{getStatusLabel(lead.status)}</span></td>
                    <td>{formatDateTime(lead.createdAt)}</td>
                    <td style={{ maxWidth: 180 }}>
                      {getLastNote(lead)?.text ? `${getLastNote(lead).text.slice(0,50)}${getLastNote(lead).text.length > 50 ? "…" : ""}` : "--"}
                    </td>
                    <td>{formatDateTime(getLastCall(lead)?.calledAt)}</td>
                    <td>
                      <button className="btn btn-primary" style={{ fontSize: 12, padding: "4px 12px" }} onClick={() => openLeadDetail(lead)}>
                        View
                      </button>
                    </td>
                  </>
